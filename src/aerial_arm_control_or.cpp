...
class CONTROLLER {
	...
	private:
        ...
        ros::Subscriber _image_fb_sub, _blob_found_sub, _tag_fb_sub, _tag_flag_sub, _vs_ee_sub;
		ros::Subscriber _ft_sensor_sub;
		ros::ServiceClient _pauseGazebo;
		ros::ServiceClient _unpauseGazebo;

		Eigen::VectorXd _arm_cmd, _f_cmd;
...
CONTROLLER::CONTROLLER() : _first_odom_1(false), _first_odom_2(false), _camera_on(false) {
	...
	// CHECK <--
	_rot_mat.resize(6,2);
	// _rot_mat   << 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3;
	_rot_mat   << 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2;
	_rot_angle.resize(6,2);
	// _rot_angle << M_PI, -M_PI/2, M_PI, -M_PI/2, M_PI, -M_PI/2, M_PI, -M_PI/2, M_PI, -M_PI/2, M_PI, -M_PI/2;
	...
	// _K_camera << 256.8370463737323,               0.0,     400.5,
	// 			    	          0.0,  256.8370463737323,    300.5,
	// 					          0.0,      		  0.0,      1.0;
...
void CONTROLLER::coupled_feedback(){
	...
	if (!_ft_sens.states.empty() ){
		// if(abs(_ft_sens.states[0].total_wrench.force.z) < 5){ //<-- fake low pass filter
...
void CONTROLLER::camera_parm_computation(){
	...
	dim_tag = 0.16;
	_pose_tag_corner_aug << _tag_pose_fb.position.x-dim_tag/2, _tag_pose_fb.position.x+dim_tag/2, _tag_pose_fb.position.x+dim_tag/2, _tag_pose_fb.position.x-dim_tag/2,
							_tag_pose_fb.position.y+dim_tag/2, _tag_pose_fb.position.y+dim_tag/2, _tag_pose_fb.position.y-dim_tag/2, _tag_pose_fb.position.y-dim_tag/2,
								      _tag_pose_fb.position.z,        _tag_pose_fb.position.z,        _tag_pose_fb.position.z,        _tag_pose_fb.position.z,
													        1,                              1,                              1,                              1;

	// Corners pixel coordinates (x,y)
	_image_fb_aug_corner.block<3,1>(0,0) = _K_camera*_P_camera*_pose_tag_corner_aug.block<4,1>(0,0);
	_image_fb_aug_corner.block<3,1>(0,0) = _image_fb_aug_corner.block<3,1>(0,0)/_image_fb_aug_corner(2,0);

	_image_fb_aug_corner.block<3,1>(0,1) = _K_camera*_P_camera*_pose_tag_corner_aug.block<4,1>(0,1);
	_image_fb_aug_corner.block<3,1>(0,1) = _image_fb_aug_corner.block<3,1>(0,1)/_image_fb_aug_corner(2,1);

	_image_fb_aug_corner.block<3,1>(0,2) = _K_camera*_P_camera*_pose_tag_corner_aug.block<4,1>(0,2);
	_image_fb_aug_corner.block<3,1>(0,2) = _image_fb_aug_corner.block<3,1>(0,2)/_image_fb_aug_corner(2,2);

	_image_fb_aug_corner.block<3,1>(0,3) = _K_camera*_P_camera*_pose_tag_corner_aug.block<4,1>(0,3);
	_image_fb_aug_corner.block<3,1>(0,3) = _image_fb_aug_corner.block<3,1>(0,3)/_image_fb_aug_corner(2,3);
	...
	// cout<<"s: "<<_s.transpose()<<endl;

	// cout<<"Corners' tag identified"<<endl;
	// cout<< "Pose: "<<_pose_tag_corner_aug<<endl;
    // cout<< "Pixel pose: "<<_image_fb_aug_corner<<endl;
    // cout<< "Pixel normalized pose: "<<_image_fb_aug_norm_corner<<endl;
	// Interaction Matrix
	_L_matrix_p1 << -1/_pose_tag_corner_aug(2,0), 							  0,  	_image_fb_aug_norm_corner(0,0)/_pose_tag_corner_aug(2,0),		_image_fb_aug_norm_corner(0,0)*_image_fb_aug_norm_corner(1,0), 							-(1+pow(_image_fb_aug_norm_corner(0,0),2)),		 _image_fb_aug_norm_corner(1,0),
											   0,  -1/_pose_tag_corner_aug(2,0),	_image_fb_aug_norm_corner(1,0)/_pose_tag_corner_aug(2,0),							(1+pow(_image_fb_aug_norm_corner(1,0),2)), 		-_image_fb_aug_norm_corner(0,0)*_image_fb_aug_norm_corner(1,0),		-_image_fb_aug_norm_corner(0,0);

	_L_matrix_p2 << -1/_pose_tag_corner_aug(2,1), 							  0,  	_image_fb_aug_norm_corner(0,1)/_pose_tag_corner_aug(2,1),		_image_fb_aug_norm_corner(0,1)*_image_fb_aug_norm_corner(1,1), 							-(1+pow(_image_fb_aug_norm_corner(0,1),2)),		 _image_fb_aug_norm_corner(1,1),
											   0,  -1/_pose_tag_corner_aug(2,1),	_image_fb_aug_norm_corner(1,1)/_pose_tag_corner_aug(2,1),							(1+pow(_image_fb_aug_norm_corner(1,1),2)), 		-_image_fb_aug_norm_corner(0,1)*_image_fb_aug_norm_corner(1,1),		-_image_fb_aug_norm_corner(0,1);

	_L_matrix_p3 << -1/_pose_tag_corner_aug(2,2), 							  0,  	_image_fb_aug_norm_corner(0,2)/_pose_tag_corner_aug(2,2),		_image_fb_aug_norm_corner(0,2)*_image_fb_aug_norm_corner(1,2), 							-(1+pow(_image_fb_aug_norm_corner(0,2),2)),		 _image_fb_aug_norm_corner(1,2),
											   0,  -1/_pose_tag_corner_aug(2,2),	_image_fb_aug_norm_corner(1,2)/_pose_tag_corner_aug(2,2),							(1+pow(_image_fb_aug_norm_corner(1,2),2)), 		-_image_fb_aug_norm_corner(0,2)*_image_fb_aug_norm_corner(1,2),		-_image_fb_aug_norm_corner(0,2);

	_L_matrix_p4 << -1/_pose_tag_corner_aug(2,3), 							  0,  	_image_fb_aug_norm_corner(0,3)/_pose_tag_corner_aug(2,3),		_image_fb_aug_norm_corner(0,3)*_image_fb_aug_norm_corner(1,3), 							-(1+pow(_image_fb_aug_norm_corner(0,3),2)),		 _image_fb_aug_norm_corner(1,3),
											   0,  -1/_pose_tag_corner_aug(2,3),	_image_fb_aug_norm_corner(1,3)/_pose_tag_corner_aug(2,3),							(1+pow(_image_fb_aug_norm_corner(1,3),2)), 		-_image_fb_aug_norm_corner(0,3)*_image_fb_aug_norm_corner(1,3),		-_image_fb_aug_norm_corner(0,3);
    ...
    // _s_dot = _J_image*vel; 
    // // cout<<"s_dot_corner_1: "<<_s_dot(0)<<", "<<_s_dot(1)<<endl;

	// cout<<"s: "<<_s.transpose()<<endl;
	// cout<<"s_dot: "<<_s_dot.transpose()<<endl;
	// --------------------------------------------------------------
	...
	corner_1 << -dim_ee/2, -dim_ee/2, 0, 1;
	corner_2 <<  dim_ee/2, -dim_ee/2, 0, 1;
	corner_3 <<  dim_ee/2,  dim_ee/2, 0, 1;
	corner_4 << -dim_ee/2,  dim_ee/2, 0, 1;

	pose_EE = _T_cb.inverse()*_T_cb_temp.inverse()*_T_arm;
	pose_EE_aug << pose_EE(0,3), pose_EE(1,3), pose_EE(2,3), 1;

	// cout<<"pose_ee: "<<pose_EE<<endl;
	...
	// cout<<"computed: "<<_EE_fb_aug_corner<<endl;
...
void CONTROLLER::arm_invdyn_control() {
	...
	// Completo 
	// sd << -0.12233,   0.452767,  -0.049245,    0.45431, -0.0456234,  0.382438,  -0.118839,   0.380565; // tag 0.02

	// multitag
	
	// sd << -0.0769684,   0.476289, -0.00843501,    0.478453, -0.00550388,    0.408387,  -0.0734679,     0.40571; 
	// sd <<  -0.120205,   0.362475, -0.0476943,   0.363537, -0.0446714,   0.293495,  -0.117656,    0.29274;
	// sd <<  -0.119223, 0.368569, -0.0409899, 0.368724, -0.0409899, 0.292155,  -0.118814, 0.291884;
	...
	while ( ros::ok() && i < N_ + 2000 ) {
		if(_start_controller){
			...
			else if(_camera_on && ibvs_flag && _iii<N_+4000 && ii>2*N_-1){
				if(_iii==0){
			...
			// --------------------- IBVS tracking task -----------------------------------
				...
				c_lambda = 0.05;
				if(_iii<N_){
					if((_iii>1000 &&  abs(e_s(0)))>0.005 && abs(e_s(0))<=0.05){
						if(_iii==1001){
							cout<<"Starting interaction control..."<<endl;
						}
						lambda_d = 0.5*(1+cos((e_s(0)-0.005)/(0.05-0.005)*M_PI));
					}
				...
				// 	// cout<<"sd("<<_iii+1<<",:) = ["<< sd(0,1) <<", "<< sd(1,1) <<", "<< sd(2,1) <<", "<< sd(3,1) <<", "<< sd(4,1) <<", "<< sd(5,1) <<", "<< sd(6,1) <<", "<< sd(7,1) <<"];"<<endl;
				// }
				if(_iii>N_){
					if(_iii==N_+1){
						cout<<"Starting Phase 3\nSliding on surface..."<<endl;
					...
					if(_iii<(N_+N_/2)){
						for(int j=0;j<8;j++){
							if(j%2==0){
								sd(j,1) = sd(j,1)-0.000075*(_iii/100-(N_)/100); 
							}
						}
						cout<<"Sliding 1"<<endl;
					}
					else if (_iii>(N_+N_/2) && _iii<2*N_) {
						for(int j=0;j<8;j++){
							if(j%2==0){	
								sd(j,1) = sd(j,1)+0.000075*(_iii/100-(N_+N_/2)/100); 
							}
						}
						cout<<"Sliding 2"<<endl;
					}
				...
				// error_temp = (J_img.transpose()*J_img).inverse()*J_img.transpose()*vs;
				J_img_temp = J_img.completeOrthogonalDecomposition().pseudoInverse();
				J_img_pinv = J_img_temp;
				error_temp = J_img_pinv*vs;
				// error_temp = (_L_matrix*_gamma_matrix.inverse()).completeOrthogonalDecomposition().pseudoInverse()*vs; //<-- check here. to modify	
				error << error_temp(3), error_temp(4), error_temp(5), error_temp(0), error_temp(1), error_temp(2);	
				// cout<<"ibvs error: "<< e_s.transpose()<<endl;
			...
			else if(i<N_+2000 && ii>2*N_-1 && !ibvs_flag) {
			// --------------------- Trajectory tracking task -----------------------------------
				...
				// switch continuo
				c_lambda = 0.05;
				if((i>1900 &&  abs(X_error(5)))>0.03 && abs(X_error(5))<=0.1){
					if(i==1901){
						cout<<"start force tracking"<<endl;
					}
					lambda_d = 0.5*(1+cos((X_error(5)-0.02)/(0.2-0.02)*M_PI));
				}
				else if(i>1900 &&  abs(X_error(5))<=0.03){
					lambda_d = 1;
				}
				else{
					lambda_d = 0;
				...
				u_q = J_wpi*(P2*u_v - J_be_d*_arm_state_vel);
