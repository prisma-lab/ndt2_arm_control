...
class CONTROLLER {
	...
	private:
		...
		// FileLog variables
		std::ofstream _epregFile;
		std::ofstream _evregFile;
...
CONTROLLER::CONTROLLER() : _first_odom_1(false), _first_odom_2(false), _camera_on(false) {
    ...
    _epregFile.open("/home/simone/uav_ws/src/ndt2_arm_control/log/epreg.txt", std::ios::out);
    _evregFile.open("/home/simone/uav_ws/src/ndt2_arm_control/log/evreg.txt", std::ios::out);
...
void CONTROLLER::camera_parm_computation(){
	...
	corner_1 << -dim_ee/2, -dim_ee/2+0.2, 0, 1.0;
	corner_2 <<  dim_ee/2, -dim_ee/2+0.2, 0, 1.0;
	corner_3 <<  dim_ee/2,  dim_ee/2+0.2, 0, 1.0;
	corner_4 << -dim_ee/2,  dim_ee/2+0.2, 0, 1.0;
	...
	_L_matrix_p1 << -1/pose_EE_aug(2), 					  0,  	_EE_fb_aug_norm_corner(0,0)/pose_EE_aug(2),		_EE_fb_aug_norm_corner(0,0)*_EE_fb_aug_norm_corner(1,0), 						 -(1+pow(_EE_fb_aug_norm_corner(0,0),2)),		 _EE_fb_aug_norm_corner(1,0),
									0,    -1/pose_EE_aug(2),	_EE_fb_aug_norm_corner(1,0)/pose_EE_aug(2),						 (1+pow(_EE_fb_aug_norm_corner(1,0),2)), 		-_EE_fb_aug_norm_corner(0,0)*_EE_fb_aug_norm_corner(1,0),		-_EE_fb_aug_norm_corner(0,0);

	_L_matrix_p2 << -1/pose_EE_aug(2), 					  0,  	_EE_fb_aug_norm_corner(0,1)/pose_EE_aug(2),		_EE_fb_aug_norm_corner(0,1)*_EE_fb_aug_norm_corner(1,1), 						 -(1+pow(_EE_fb_aug_norm_corner(0,1),2)),		 _EE_fb_aug_norm_corner(1,1),
									0,    -1/pose_EE_aug(2),	_EE_fb_aug_norm_corner(1,1)/pose_EE_aug(2),						 (1+pow(_EE_fb_aug_norm_corner(1,1),2)), 		-_EE_fb_aug_norm_corner(0,1)*_EE_fb_aug_norm_corner(1,1),		-_EE_fb_aug_norm_corner(0,1);

	_L_matrix_p3 << -1/pose_EE_aug(2), 					  0,  	_EE_fb_aug_norm_corner(0,2)/pose_EE_aug(2),		_EE_fb_aug_norm_corner(0,2)*_EE_fb_aug_norm_corner(1,2), 						 -(1+pow(_EE_fb_aug_norm_corner(0,2),2)),		 _EE_fb_aug_norm_corner(1,2),
									0,    -1/pose_EE_aug(2),	_EE_fb_aug_norm_corner(1,2)/pose_EE_aug(2),						 (1+pow(_EE_fb_aug_norm_corner(1,2),2)), 		-_EE_fb_aug_norm_corner(0,2)*_EE_fb_aug_norm_corner(1,2),		-_EE_fb_aug_norm_corner(0,2);

	_L_matrix_p4 << -1/pose_EE_aug(2), 					  0,  	_EE_fb_aug_norm_corner(0,3)/pose_EE_aug(2),		_EE_fb_aug_norm_corner(0,3)*_EE_fb_aug_norm_corner(1,3), 						 -(1+pow(_EE_fb_aug_norm_corner(0,3),2)),		 _EE_fb_aug_norm_corner(1,3),
									0,    -1/pose_EE_aug(2),	_EE_fb_aug_norm_corner(1,3)/pose_EE_aug(2),						 (1+pow(_EE_fb_aug_norm_corner(1,3),2)), 		-_EE_fb_aug_norm_corner(0,3)*_EE_fb_aug_norm_corner(1,3),		-_EE_fb_aug_norm_corner(0,3);
...
void CONTROLLER::new_plan() {
	...
	// while (ros::ok()) {
		...
		if(!_traj_ready){
			...
			theta_goal << -0.2681270250000001, -0.43289869999999997, 1.3734638000000001, -0.0018849000000003002, -0.6101999999999999, 0.0;
			...
			theta_s << 0.0, -0.5076663999999997, 1.1472757999999996, 0.0018848999999998561, -0.6145200000000002, 0.0;
			T_s = dir_kin(theta_s,_S_axis,_M);
...
void CONTROLLER::arm_invdyn_control() {
	...
	sd << -0.146116, -0.232102,  0.156216, -0.231255,  0.158847, -0.534022,  -0.14591, -0.534183; // tag 0.08
	...
	while ( ros::ok() && i < N_+4000 ) {
		if(_start_controller){
			...
			if(ii<N_ && !_start_vs){
				...
				ev = - _arm_state_vel;
				writedata(ep,_epregFile);
				writedata(ev,_evregFile);
			...
			else if(i<N_+4000 && ii>N_-1 && !ibvs_flag) {
				if(i==0){
					cout<<"Starting Phase 2...\nCartersian trajectory tracking..."<<endl;
				}
			// --------------------- Trajectory tracking task -----------------------------------
				...
				// switch continuo
				c_lambda = 0.05;
				if((i>2000 &&  abs(X_error(5)))>0.03 && abs(X_error(5))<=0.1){
					lambda_d = 0.5*(1+cos((X_error(5)-0.02)/(0.2-0.02)*M_PI));
				}
				else if(i>2000 &&  abs(X_error(5))<=0.03){
					lambda_d = 1;
				}
				else{
					lambda_d = 0;
