...
class CONTROLLER {
	public:
		...
		// Subscribers Callback fun
	    	void ft_sens_cb( const geometry_msgs::WrenchStamped ft_sens_msg );
	...
	private:
		...
		geometry_msgs::WrenchStamped _ft_sens;
		int _rate;
		// Interaction forces
...
CONTROLLER::CONTROLLER() {
	...
	// ROS subscribers
	_arm_ft_sub  = _nh.subscribe("/force_sensor", 1, &CONTROLLER::ft_sens_cb, this);
	_traj_sub  = _nh.subscribe("/ndt2/trajectory/lin_pos", 1, &CONTROLLER::traj_cb, this);
    
	_fFile.open("/home/simone/uav_ws/src/ndt2_arm_control/log/hitl/f_babyk.txt", std::ios::out);
	_tauFile.open("/home/simone/uav_ws/src/ndt2_arm_control/log/hitl/tau_babyk.txt", std::ios::out);
...
void CONTROLLER::ft_sens_cb( const geometry_msgs::WrenchStamped ft_sens_msg ){
...
void CONTROLLER::loop() {
	...
	while ( ros::ok() ) {
        ...
        _f_ext << _ft_sens.wrench.force.x,_ft_sens.wrench.force.y,_ft_sens.wrench.force.z;
        _m_ext << _ft_sens.wrench.torque.x,_ft_sens.wrench.torque.y,_ft_sens.wrench.torque.z;
        _m_ext = R_rot*_m_ext;
		_f_ext = R_rot*_f_ext;
	...
	writedata(_m_ext,_tauFile);
	writedata(_f_ext,_fFile);
